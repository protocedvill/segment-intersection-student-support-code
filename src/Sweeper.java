...
public class Sweeper {
  ...
  private OrderedSet<LineSegment> segments_in_sweep;          // those segments that intersect the sweep line

  public Sweeper(List<LineSegment> world, int treeType) {
    this.world = world;
    /**
     * Note that the relationship on which the tree is organized changes with the
     * changing sweep line. This means we have to rebuild if we want another
     * intersection (and to mitigate the effect of the dirty segments in the tree).
     */
    BiPredicate<LineSegment, LineSegment> sweepPred = 
        (LineSegment s1, LineSegment s2) -> s1.lessThan(s2, sweepX);
    if (treeType == Constants.BST)
      segments_in_sweep = new BinarySearchTree<>(sweepPred);
    else 
      segments_in_sweep = new AVLTree<>(sweepPred);
  }
   ...
   * detected, in which case true is returned, or the rightmost endpoint
   * is processed, in which case false is returned.
   */
  public boolean run() {
    ...
    for (Endpoint currEndpoint : endpoints) {
      ...
      if (currEndpoint.onLeft()) {
        ...
        // Add the segment to the tree because it intersects the sweep line.
        Location<LineSegment> loc = segments_in_sweep.insert(currSeg);
        Location<LineSegment> above = loc.next();
        if (above != null) {
        ...
        Location<LineSegment> below = loc.previous();
        if (below != null) {
        ...
        Location<LineSegment> loc = segments_in_sweep.search(currSeg);
        if (loc == null) {
          if ((loc = segments_in_sweep.search(currSeg)) == null)
            throw new SweeperException(currSeg, segments_in_sweep);  // shouldn't happen, but just in case...
        }
        Location<LineSegment> above = loc.next();
        Location<LineSegment> below = loc.previous();
        segments_in_sweep.remove(currSeg);
...
/**
 * This exception is useful for debugging purposes. It is caught by the
 * GUI, which highlights the segment and the sweep line and also prints
...
class SweeperException extends RuntimeException {
  ...
  private OrderedSet<LineSegment> tr;
  
  public SweeperException(LineSegment seg, OrderedSet<LineSegment> tr) {
    super("Sweeper: the search failed to locate " + seg + 
        " in the tree while processing its right endpoint.");
    this.seg = seg;
    this.tr = tr;
  }
  ...
  public OrderedSet<LineSegment> getTree() {
    return tr;
  }
}
