 ...
 * [read-only]
 * 
 * A OrderedSet is searchable. The result of performing a search on a tree 
 * is a Location. (In the case of BinarySearchTree, the Location will
 * be a Node.) For convenience, we've modified the signature of insert()
 * to return the location of the key that was inserted. Additional
 * operations include a clear() method and a keys() method.
 */
...
public interface OrderedSet<K> {
  ...
  /**
   * Returns the height of this tree, where height is the number of nodes
  ...
  /**
   * Searches the tree for the key and, if found, returns a "pointer" to where 
   * it is. Otherwise, null is returned. 
   */
